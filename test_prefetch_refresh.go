...
func main() {
	...
	fmt.Println("✓ Proxy created with aggressive prefetch settings:")
	...
	fmt.Println("\n✓ Domain should now be in prefetch queue")
	fmt.Println()
	...
	for monitorCount < maxMonitors {
		...
		if dctx.Res != nil && len(dctx.Res.Answer) > 0 {
			...
			if monitorCount > 3 && ttl > 200 {
				fmt.Println("\n✓ TTL appears to have been refreshed by prefetch!")
	...
	fmt.Println()
	fmt.Println("Summary:")
	fmt.Println("✓ Domain was added to prefetch queue after 2 accesses")
	fmt.Println("✓ Prefetch scheduler monitored cache TTL")
	fmt.Println("✓ Cache was automatically refreshed before expiration")
	fmt.Println("✓ Continuous access pattern maintained cache freshness")
	fmt.Println()
